...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_D)) jogador.x += vel;
        if (IsKeyDown(KEY_LEFT)  || IsKeyDown(KEY_A)) jogador.x -= vel;
        if (IsKeyDown(KEY_UP)    || IsKeyDown(KEY_W)) jogador.y -= vel;
        if (IsKeyDown(KEY_DOWN)  || IsKeyDown(KEY_S)) jogador.y += vel;
        ...
        BeginDrawing();
            ...
            DrawText("C cura em area todos os inimigos vivos (+15)", 10, 35, 18, DARKGREEN);
            DrawText("Setas/WASD movem o jogador | ESC sai", 10, ALTURA_JANELA - 25, 16, GRAY);
